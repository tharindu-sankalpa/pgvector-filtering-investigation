...
To find available database configurations:
    1. Export environment variables:
       export $(grep -v '^#' .env.azure | xargs)
       
    2. Connect to the results database using pg_cli:
       PGSSLMODE=require PGHOST=$PG_HOST PGUSER=$PG_USER PGPASSWORD=$PG_PASSWORD PGDATABASE=benchmark_results psql
       
    3. Run the following SQL to list configs for CNPG:
       SELECT DISTINCT database_config 
       FROM benchmark_retrieval_summary 
       WHERE database_name = 'PostgreSQL CNPG Self-Hosted';

Usage:
    export $(grep -v '^#' .env.azure | xargs)
    uv run python scripts/plot_article_benchmarks.py "<target_database_config>"

    Example:
    uv run python scripts/plot_article_benchmarks.py "D16s_v3 16vCPU 64GB | asyncpg - WoT Dataset 2.5M"

Output:
    doc/plots/article_cnpg_retrieval_<slugified_config>.png
"""
...
RESULTS_PG_HOST = os.getenv("RESULTS_PG_HOST", os.getenv("PG_HOST", os.getenv("PGHOST", "")))
RESULTS_PG_USER = os.getenv("RESULTS_PG_USER", os.getenv("PG_USER", os.getenv("PGUSER", "")))
RESULTS_PG_PASSWORD = os.getenv("RESULTS_PG_PASSWORD", os.getenv("PG_PASSWORD", os.getenv("PGPASSWORD", "")))
RESULTS_PG_DATABASE = "benchmark_results"


def fetch_cnpg_results(database_config: str = None) -> pd.DataFrame:
    """Fetch CNPG 2.5M retrieval results, picking the latest run per scenario."""
    conninfo = (
    ...
    # Use DISTINCT ON to pick the *latest* row per scenario based on run_timestamp
    query = """
    ...
    if database_config:
        query += " AND database_config = %s"
        params.append(database_config)

    query += " ORDER BY test_type, top_k, concurrency_level, run_timestamp DESC"
    
    rows = conn.execute(query, params).fetchall()
    cols = [
        "test_type", "top_k", "concurrency_level",
        "avg_latency_ms", "p50_ms", "p95_ms", "p99_ms", "qps", "database_config", "run_timestamp"
    ]
    conn.close()
...
def plot_dashboard_style(df: pd.DataFrame, database_config: str = None) -> None:
    """
    ...
    config_title_str = ""
    if database_config:
        config_title_str = f" - Config: {database_config}"
        
    fig.suptitle(
        f"PostgreSQL CNPG Self-Hosted Retrieval Performance (2.5M Vectors){config_title_str}",
        fontsize=15, fontweight="bold", color=text_color, y=0.98,
    )

    filename = "article_cnpg_retrieval.png"
    if database_config:
        safe_config_name = database_config.replace(" | ", "_").replace(" - ", "_").replace(" ", "_").lower()
        filename = f"article_cnpg_retrieval_{safe_config_name}.png"
...
def main() -> None:
    ...
    logger.info("plot_generation_start", database_config=database_config)

    df = fetch_cnpg_results(database_config=database_config)
    
    if df.empty:
        logger.error("no_data_found_for_given_config", database_config=database_config)
        return
    ...
    plot_dashboard_style(df, database_config=database_config)
